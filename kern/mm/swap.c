...
#define CHECK_VALID_VADDR ((CHECK_VALID_VIR_PAGE_NUM+1)*(BEING_CHECK_VALID_VADDR))
...
volatile int swap_page[CHECK_VALID_VIR_PAGE_NUM];
volatile int swap_in_seq_no[MAX_SEQ_NO], swap_out_seq_no[MAX_SEQ_NO];
...
int swap_in(struct mm_struct *mm, uintptr_t addr, struct Page **ptr_result){
    ...
    //cprintf("swap in: load disk swap entry %d with swap_page in vaddr 0x%x", (*ptep) >> 8, addr);
    *ptr_result = result;
    return 0;
}
